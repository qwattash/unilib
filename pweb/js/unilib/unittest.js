...
testManager = {
  ...
  /**
   * get index of given test inside this.asyncTests
   * @param {Test} test
   * @return {Number} index || -1 if not found
   * @private
   */
  getAsyncTestIndex_: function(test) {
    return this.asyncTests.indexOf(test);
  },
   ...
   */
  registerTest: function(test, isAsync) {
    //prevent duplicates, even if it is unlikely that there will be any
    if (isAsync) {
      if (this.asyncTests.indexOf(test) == -1) {
        this.asyncTests.push(test);
      }
    }
    else {
  ...
  /**
   * execute asynchronous tests registered
   * @public
   */
  executeAsync: function() {
    if (this.asyncTests.length == 0) return;
    this.executeAsyncStep(0);
  },
  
  /**
   * async test execution helper, execute next step
   * @param {number} index of the test to executed in this.asyncTests
   */
  nextStep: function(index) {
    index++;
    if (index < this.asyncTests.length) {
      this.executeAsyncStep(index);
    }
  },
  
  /**
   * helper for async test execution, execute a test and wait a timeout at the 
   * end before executing the next one
   * @param {number} index of the test to execute in this.asyncTests
   */
  executeAsyncStep: function(index) {
    this.current = this.asyncTests[index];
    this.startTest(this.current);
    this.current.run();
    if (this.current.timeout >= 0) {
      if (this.current.total < this.current.expected) {
        window.setTimeout(
          unilib.createCallback(this, this.checkAsyncResults, [index]), 
      ...
      this.checkAsyncResults(index);
    }
  },
  
  /**
   * helper of async test execution, check results
   * @param {number} index of the test to execute in this.asyncTests
   */
  checkAsyncResults: function(index) {
    if (this.current.total < this.current.expected) {
      if (this.current.timeout >= 0) {
        this.endTest();
        this.nextStep(index);
 ...
 * @param {string} testName
 * @param {Function} body
 * @param {number} expected expected number of assertions
 * @param {number} [timeout=10000] a negative value make the wait
 *  infinite
 */
function AsyncTest(testName, body, expected, timeout) {
  Test.call(this, testName, body);
  ...
  /**
   * timeout to wait at the end of the test
   * @type {number}
   */
  this.timeout = (timeout == undefined) ? 10000 : timeout;
};
unilib.inherit(AsyncTest, Test.prototype);

/**
 * delegate startTest and endTest to testManager
 * @see {Test#run}
 */
...
// assertion helpers grouped in a namespace
/**
 * @private
    ...
    testDeepEqualObject: function(value, expected, result) {
      if (typeof value == 'object' && typeof expected == 'object') {
        if (value === null && expected !== null ||
          ...
          result.resultCode = Result.FAIL;
          result.errorMessage = 'found null value';
          return;
        }
...
function test(testName, body) {
  ...
  testManager.registerTest(testObject, false);
}

/**
 * create an AsyncTest and add it to the testManager
 * @param {string} testName
 * @param {number} expected
 * @param {number} timeout
 * @param {Function} body
 */
function asyncTest(testName, expected, timeout, body) {
  var testObj = new AsyncTest(testName, body, expected, timeout);
  testManager.registerTest(testObj, true);
}
